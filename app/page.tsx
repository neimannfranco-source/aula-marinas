...
export default function Home() {
    ...
    (async () => {
      ...
      try {
        ...
        if (saved) {
          ...
          setAppState({ ...createInitialState(), ...p, currentStudentId: null });
        } else {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try { localStorage.setItem(LS_KEY, JSON.stringify(appState)); } catch {}
      try { await saveRemoteState(appState); } catch {}
    }, 500);
    return () => clearTimeout(t);
  }, [appState, loadStatus]);

  const logout = () => {
    setAppState((p) => ({ ...p, currentStudentId: null }));
  ...
  const handleProfessorClick = () => {
    if (profUnlocked) { setShowProfPanel((v) => !v); return; }
  ...
  const changePwd = () => {
    if (!newPwd.trim()) { setPwdMsg("Digite uma nova senha."); return; }
    if (newPwd.trim().length < 4) { setPwdMsg("Mínimo 4 caracteres."); return; }
    if (newPwd.trim() !== confirmPwd.trim()) { setPwdMsg("As senhas não coincidem."); return; }
    if (!currentStudent) return;
    setAppState((p) => ({
      ...p,
      students: p.students.map((s) =>
        s.id === currentStudent.id ? { ...s, code: newPwd.trim().toUpperCase() } : s
      ),
    }));
  ...
  if (loadStatus === "loading") {
    return (
      ...
      >
        ...
        `}</style>
        <div style={{ width: 48, height: 48, border: `3px solid ${C.border}`, borderTop: `3px solid ${C.green}`, borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
        <span style={{ color: C.textDim, fontSize: 13 }}>Carregando Aula Marinas...</span>
      </div>
    );
  }

  // Login / professor panel (not logged in)
  if (!currentStudent) {
    return (
      <>
        <Login
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: FONT }}>
      ...
      >
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginRight: 4 }}>
            ...
            <span style={{ fontWeight: 800, fontSize: 15, letterSpacing: "-0.02em", fontFamily: DISPLAY, color: C.text }}>
              Aula Marinas
            </span>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <button
              onClick={() => { setShowChangePwd((v) => !v); setShowProfPanel(false); }}
              style={{ ...btnGhost, padding: "5px 10px", fontSize: 13 }}
            >
              🔑
            </button>
            <button onClick={handleProfessorClick} style={{ ...btnGhost, padding: "5px 10px", fontSize: 13 }}>
              👨‍🏫
            </button>
            <div
              style={{
        ...
        {showChangePwd && (
          <div style={{ background: C.bg2, borderTop: `1px solid ${C.border}`, padding: "14px 24px" }}>
            <div style={{ maxWidth: 500, display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
              <input
                type="password"
                value={newPwd}
              ...
              />
              <button onClick={changePwd} style={btnPrimary}>Salvar</button>
              <button onClick={() => { setShowChangePwd(false); setPwdMsg(""); }} style={btnGhost}>Cancelar</button>
              {pwdMsg && (
                <span style={{ fontSize: 13, color: pwdMsg.startsWith("✓") ? C.green : C.red, width: "100%" }}>
                  {pwdMsg}
                </span>
              )}
            </div>
      ...
      {/* Professor panel (inline below header) */}
      {showProfPanel && profUnlocked && (
